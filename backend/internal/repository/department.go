...
func NewDepartmentRepository(db *db.PostgresDB) domain.DepartmentRepository {
...
func (d *departmentRepository) Create(ctx context.Context, department *domain.Department) error {
	...
	executor := d.db.GetDB(ctx)
	if err := executor.QueryRow(ctx, query, department.Name, department.CreatedAt, department.UpdatedAt).Scan(&department.ID); err != nil {
...
func (d *departmentRepository) Update(ctx context.Context, department *domain.Department) error {
	...
	executor := d.db.GetDB(ctx)
	result, err := executor.Exec(ctx, query, department.Name, department.UpdatedAt, department.ID)
	if err != nil {
		return apperror.NewPostgresError(err)
	}
...
func (d *departmentRepository) GetList(ctx context.Context, filter domain.DepartmentFilter) ([]*domain.Department, int64, error) {
	...
	return getList(ctx, d.db, sbFilter, sbCount, func(scanner rowScanner) (*domain.Department, error) {
