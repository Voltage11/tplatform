  ...
  (newVal) => {
    if (newVal) {
      if (props.editingDepartment) {
        name.value = props.editingDepartment.name
      } else {
        name.value = ''
      }
      clearErrors()
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.editingDepartment) {
      await store.updateDepartment(props.editingDepartment.id, name.value)
    } else {
      await store.createDepartment(name.value)
    }
    emit('saved')
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="modal-actions">
          ...
          <button type="submit" class="btn-save" :disabled="isSubmitting">
            {{ isSubmitting ? 'Сохранение...' : editingDepartment ? 'Обновить' : 'Создать' }}
          </button>
        </div>
      </form>
    </div>
